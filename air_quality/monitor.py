...
def cusum(
    ...
    uncertainty: matrix of prediction uncertainties with the same shape
        as ``observed``; each value must additionally be >= 0.
    drift: non-bool finite int or float >= 0; amount subtracted from
        each step's excess error before accumulation.
    threshold: non-bool finite int or float >= 0; a CUSUM level at or
        above it counts as a trigger.
    tolerance: non-bool finite int or float >= 0 (default 0.0); absolute
        errors up to ``tolerance + uncertainty[t][i]`` are disregarded.
    ...
    drift_value = _validate_non_negative("drift", drift)
    threshold_value = _validate_non_negative("threshold", threshold)
