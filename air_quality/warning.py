...
def transition(
    ...
    """Summarize warning-level transitions along the upper confidence band.
    ...
    K-long plain list of ints in time order; ``up`` is the number of
    ``t`` in 1..K-1 with ``level[t] > level[t-1]`` and ``down`` the
    number with ``level[t] < level[t-1]``; ``peak`` is ``max(levels)``
    and ``peak_time`` is the smallest index at which ``peak`` first
    occurs. All summary values are ints and results are not rounded.
    """
    val_matrix = _validate_matrix("values", values, allow_negative=False)
    unc_matrix = _validate_matrix("uncertainty", uncertainty, allow_negative=False)
    if len(unc_matrix) != len(val_matrix):
