...
def window(
    ...
    """Find sustained maximum-level warning windows over time.
    ...
    thresholds: list or tuple of 3 strictly increasing, finite,
        non-negative warning thresholds.
    z: finite, non-negative confidence multiplier (default 1.96).
    minimum: non-bool int >= 1 giving the run length that qualifies a
        warning window (default 1).
    ...
    Times with ``level[t] == 3`` mark maximum-level warnings; adjacent
    marked times form runs.

    Returns ``(levels, longest, first_start, qualifying_runs)`` where
    ``levels`` is a K-long plain list of ints in time order, ``longest``
    is the length of the longest run (0 when there are no runs),
    ``first_start`` is the smallest start index of a longest run (-1 when
    there are no runs) and ``qualifying_runs`` is the number of runs with
    length >= ``minimum``. Results are not rounded.
    """
    val_matrix = _validate_matrix("values", values, allow_negative=False)
    unc_matrix = _validate_matrix("uncertainty", uncertainty, allow_negative=False)
    ...
    for t in range(len(val_matrix)):
        ...
        level = sum(1 for t_value in levels_thresholds if t_value <= high_score)
        levels.append(level)
