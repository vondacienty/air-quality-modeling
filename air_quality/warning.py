...
def alert_run_profile(
    ...
    """Time profile of completed runs of consecutive top-level alerts.
    ...
    of floats in time order; ``first[t]`` is the probability that a run
    of ``m`` consecutive top-level alerts is first completed exactly at
    time ``t`` and ``cumulative_any[t]`` is the probability that at least
    one qualifying run has been completed by time ``t`` (inclusive).
    ``distribution`` is a (K + 1)-long plain list of floats with
    ``distribution[c]`` the probability of completing exactly ``c``
    qualifying runs within the horizon, and ``expected_runs`` is the
    float ``fsum(c * distribution[c] for c in range(K + 1))``. Results
    are not rounded.
    """
    _, _, q = level_probability(values, uncertainty, population, beta, thresholds)
    ...
    for q_t in q:
        ...
        for r, c in sorted(state):
            ...
            if r < m:
                ...
                if completes and c == 0:
                    first_contributions.append(p * q_t)
            else:
                key = (r, c)
        ...
        first.append(math.fsum(first_contributions))
        cumulative_any.append(
            math.fsum(p for (_, c), p in sorted(state.items()) if c >= 1)
        )

    grouped: dict[int, list[float]] = {}
    for (_, c), p in sorted(state.items()):
        grouped.setdefault(c, []).append(p)
    distribution = [0.0] * (k_times + 1)
    for c, values in grouped.items():
        distribution[c] = math.fsum(values)
    expected_runs = math.fsum(
        c * distribution[c] for c in range(k_times + 1)
    )
...
def alert_level_runs(
    ...
    """Time profile of completed runs of consecutive alerts for each level.
    ...
    ``p`` is exactly the first item (``probabilities``) returned by
    ``level_probability(values, uncertainty, population, beta,
    thresholds)``: ``p[t][j]`` is the probability that the level at time
    ``t`` is exactly level ``j`` (levels 0..3). For each threshold index
    ``j`` in 1..3 the alert event "level at least ``j``" at time ``t`` is
    treated as an independent Bernoulli event with probability::

        q[t][j - 1] = fsum(p[t][j:])

    Writing ``m = minimum``, the state ``(r, c)`` (one distribution per
    threshold level) tracks the current run length ``r`` of consecutive
    alerts truncated to ``m`` and the count ``c`` of completed qualifying
    runs; the initial state is ``P(0, 0) = 1``. For each time ``t``:
    ...
    ``first`` and ``cumulative`` are K x 3 plain lists of lists of floats
    in time order (one column per threshold level); ``first[t][j - 1]`` is
    the probability that a run of ``m`` consecutive level-``j``-or-higher
    alerts is first completed exactly at time ``t`` and
    ``cumulative[t][j - 1]`` is the probability that at least one such run
    has been completed by time ``t`` (inclusive). ``expected_runs`` is a
    3-long plain list of floats with
    ``expected_runs[j - 1] = fsum(c * P(r, c) for (r, c) in states)``.
    Results are not rounded.
    """
    p, _, _ = level_probability(
    ...
    m = minimum
    q: list[list[float]] = [
        [math.fsum(p_t[j:]) for j in (1, 2, 3)] for p_t in p
    ]

    first: list[list[float]] = [[] for _ in range(len(q))]
    cumulative: list[list[float]] = [[] for _ in range(len(q))]
    ...
    for j in range(3):
        ...
        for t in range(len(q)):
            ...
            first[t].append(math.fsum(first_contributions))
            cumulative[t].append(
                math.fsum(
                    p_state
        ...
        expected_runs.append(
            math.fsum(c * p_state for (r, c), p_state in state.items())
        )
...
def alert_level_distribution(
    ...
    """Distribution of completed runs of consecutive alerts for each level.
    ...
    Returns ``(q, distribution, expected_runs)`` where ``q`` is a K x 3
    plain list of lists of floats in time order (one column per threshold
    level); ``distribution`` is a 3 x (K + 1) plain list of lists of
    floats with ``distribution[j - 1][c]`` the probability of completing
    exactly ``c`` qualifying runs of level-``j``-or-higher alerts within
    the horizon; ``expected_runs`` is a 3-long plain list of floats with
    ``expected_runs[j - 1] = fsum(c * distribution[j - 1][c] for c in
    range(K + 1))``. Results are not rounded.
    """
    p, _, _ = level_probability(
        values, uncertainty, population, beta, thresholds
    ...
    for j in range(3):
        ...
        level_distribution = [0.0] * (k_times + 1)
        grouped: dict[int, list[float]] = {}
        for (_, c), p_state in sorted(state.items()):
            grouped.setdefault(c, []).append(p_state)
        for c, contribs in grouped.items():
            level_distribution[c] = math.fsum(contribs)
        distribution.append(level_distribution)
        expected_runs.append(
            math.fsum(
                c * level_distribution[c] for c in range(k_times + 1)
            )
        )
...
def alert_level_profile(
    ...
    """Per-time profile of completed-run counts for each threshold level.
    ...
    thresholds: list or tuple of 3 strictly increasing, finite,
        non-negative warning thresholds.
    minimum: non-bool int >= 1 giving the required run length of
        consecutive threshold exceedances (default 1).
    ...
    The three threshold exceedance events across time are treated as
    independent Bernoulli events. Writing ``m = minimum``, the state
    ``(r, c)`` (one distribution per threshold) tracks the current run
    length ``r`` of consecutive exceedances truncated to ``m`` and the
    count ``c`` of completed qualifying runs; the initial state is
    ``P(0, 0) = 1``. For each time ``t``:

        exceedance (probability q[t][j]):
            (r, c) -> (r + 1, c + [r + 1 == m])   if r < m
            (r, c) -> (r, c)                      if r == m
        no exceedance (probability 1 - q[t][j]):
            (r, c) -> (0, c)
    ...
    floats in time order; ``D`` is a K x 3 x (K + 1) plain list with
    ``D[t][j][c]`` the probability, after time ``t``, of having completed
    exactly ``c`` qualifying runs, i.e. ``fsum(P(r, c) over r in
    ascending order)``; ``E`` is a K x 3 plain list of lists of floats
    ...
    D: list[list[list[float]]] = [
        [[0.0] * (k_times + 1) for _ in range(3)] for _ in range(k_times)
    ]
    ...
    for j in range(3):
        ...
        for t in range(k_times):
            ...
            for c, entries in grouped.items():
                ...
                D[t][j][c] = math.fsum(p_state for _, p_state in entries)
            E[t][j] = math.fsum(
