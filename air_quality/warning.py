...
def forecast_interval(
    ...
    """Forecast interval-based air quality warning levels over time.
    ...
    thresholds: list or tuple of 3 strictly increasing, finite,
        non-negative warning thresholds.
    z: finite, non-negative interval half-width multiplier (default 1.96).
    Returns ``(low_levels, high_levels, low_scores, high_scores)``, plain
    K-length lists in time order, where for each time ``t`` and receptor
    ``i``::
    ...
    and ``low_scores[t] = L[t]``, ``high_scores[t] = H[t]``. Results are
    not rounded.
    """
    val_matrix = _validate_matrix("values", values, allow_negative=False)
    unc_matrix = _validate_matrix("uncertainty", uncertainty, allow_negative=False)
    ...
    for t in range(len(val_matrix)):
        lo_row = [max(val_matrix[t][i] - z_value * unc_matrix[t][i], 0.0)
                  for i in range(n_receptors)]
        hi_row = [val_matrix[t][i] + z_value * unc_matrix[t][i]
                  for i in range(n_receptors)]
        low_score = math.fsum(populations[i] * beta_value * lo_row[i]
                              for i in range(n_receptors))
        high_score = math.fsum(populations[i] * beta_value * hi_row[i]
                               for i in range(n_receptors))
        low_levels.append(
