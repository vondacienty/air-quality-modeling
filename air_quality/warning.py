"""Health warning levels driven by health impact assessment results."""
...
def _validate_matrix(name: str, matrix: object, nonnegative: bool) -> list[list[float]]:
...
def _validate_thresholds(thresholds: object) -> list[float]:
    if not isinstance(thresholds, (list, tuple)):
        raise TypeError(
            "thresholds must be a list or tuple, got "
            f"{type(thresholds).__name__}"
        )
    if len(thresholds) != 3:
        raise ValueError(f"thresholds must have 3 elements, got {len(thresholds)}")
    validated = []
    for j, item in enumerate(thresholds):
        if not _is_number(item):
            raise TypeError(
                f"thresholds[{j}] must be an int or float, got {type(item).__name__}"
            )
        value = float(item)
    ...
    for j in range(1, 3):
        if not validated[j] > validated[j - 1]:
            raise ValueError(
                "thresholds must be strictly increasing, got "
                f"{validated[j - 1]!r} followed by {validated[j]!r}"
            )
    return validated
...
def assess(
    ...
    """Assign health warning levels from impact estimates and uncertainties.
    ...
    H: non-empty scenario x receptor matrix of health impacts; both the
        ...
        non-empty and share one receptor count; each value must be finite
        and may be negative.
    W: matrix with the same shape as ``H`` of standard uncertainties; each
    ...
    * ``I = fsum(max(H[k][i], 0) for i)`` is the positive health impact,
    * ``V`` is the combined uncertainty, accumulating ``W[k]`` with
      ``math.hypot`` (equivalent to ``sqrt(fsum(W[k][i] ** 2))`` without
      intermediate overflow),
    * ``scores[k] = I + V``,
    * ``levels[k]`` is the count of thresholds ``<= scores[k]``, i.e. an
      int in 0..3,
    * ``evidence[k] = (I, V)``.

    The baseline scenario returns ``level 0``, ``evidence (0.0, 0.0)`` and
    ``score 0.0``. All results are plain lists in input order and are not
    rounded.
    """
    impacts = _validate_matrix("H", H, nonnegative=False)
    uncertainties = _validate_matrix("W", W, nonnegative=True)

    if len(uncertainties) != len(impacts):
        raise ValueError(
            "H and W must have the same number of scenarios, "
    ...
    cutoffs = _validate_thresholds(thresholds)
    ...
    for k in range(len(impacts)):
        if k == baseline:
            ...
            scores.append(0.0)
            continue
        impact = math.fsum(max(h, 0.0) for h in impacts[k])
        variance = 0.0
        for w in uncertainties[k]:
            variance = math.hypot(variance, w)
        score = impact + variance
        level = sum(1 for cutoff in cutoffs if cutoff <= score)
        levels.append(level)
        evidence.append((impact, variance))
        scores.append(score)
