...
def risk_contribution(
    ...
    """Per-scenario weighted exceedance probability and expected excess.
    ...
    * when ``sigma[k] > 0``, with ``a = (thresholds[j] - mu[k]) / sigma[k]``:
      ...
      ``e = sigma[k] * exp(-a * a / 2) / sqrt(2 * pi)
      + (mu[k] - thresholds[j]) * q``;
    * when ``sigma[k] == 0``: ``q = 1.0`` if
      ``thresholds[j] <= mu[k]`` else ``0.0``, and
      ``e = max(mu[k] - thresholds[j], 0.0)``;
    * ``P[k][j] = w[k] * q`` and ``E[k][j] = w[k] * e``.
