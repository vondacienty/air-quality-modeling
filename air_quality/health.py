...
def receptor_count_interval(
    ...
    """Mean and interval for the number of receptors exceeding thresholds.
    ...
    * ``c[k][j] = fsum(q(k, i, j) for i in range(N))`` is the expected
      number of exceeding receptors for scenario ``k`` and threshold
      ``j``;
    * ``v[k][j] = fsum(q(k, i, j) * (1 - q(k, i, j)) for i in range(N))``
      is its binomial variance;
    * ``mean[j] = fsum(w[k] * c[k][j] for k in range(K))``;
    * ``V[j] = fsum(w[k] * ((c[k][j] - mean[j]) ** 2 + v[k][j])
      for k in range(K))``;
    * ``spread[j] = z * sqrt(V[j])``;
    * ``lower[j] = max(0, mean[j] - spread[j])`` and
      ``upper[j] = min(N, mean[j] + spread[j])``.
        ...
        for level in levels:
            ...
            for i in range(n_receptors):
                ...
                if sigma > 0:
                    ...
                    q = 1.0 if level <= impacts[k][i] else 0.0
                qs.append(q)
            c_row.append(math.fsum(qs))
            v_row.append(math.fsum(q * (1.0 - q) for q in qs))
        counts.append(c_row)
        variances.append(v_row)

    M = [
        math.fsum(w[k] * counts[k][j] for k in range(n_scenarios))
        for j in range(3)
    ]
    V = [
        math.fsum(
            w[k] * ((counts[k][j] - M[j]) ** 2 + variances[k][j])
            for k in range(n_scenarios)
        )
        for j in range(3)
    ]
    R = [z * math.sqrt(variance) for variance in V]
    lower = [max(0.0, M[j] - R[j]) for j in range(3)]
    upper = [min(float(n_receptors), M[j] + R[j]) for j in range(3)]

    return M, R, lower, upper
