...
def receptor_level_count_rise_distribution(
    ...
    """Distribution of the number of rises in the count at a health level.

    Scenario errors are treated as independent and receptors are folded in
    their given order. A "rise" is a step from scenario ``k - 1`` to
    scenario ``k`` in which the number of receptors at ``level`` strictly
    increases.
    ...
    * ``p[k][i] = [1 - q0, q0 - q1, q1 - q2, q2]`` holds the probabilities
    ...
    For each scenario ``k`` the receptors are folded into the count PMF
    ``Ck``: starting from ``d = [1] + [0] * N``, each receptor ``i`` with
    ``p = p[k][i][level]`` updates
    ``d'[r] = fsum((d[r] * (1 - p), d[r - 1] * p if r > 0 else 0))`` and
    ``Ck = d``. The recursion then starts from ``D[(c, 0)] = C0[c]`` and,
    for ``k = 1..K - 1``, every state ``(c, r)`` (visited together with the
    target count ``c'`` in lexicographic order) spreads its mass over
    ``D'[(c', r + (c' > c))] += D[(c, r)] * Ck[c']``, contributions to the
    same target being merged with ``math.fsum``.

    Returns ``R[r] = fsum(D[(c, r)] for c in 0..N)``, a K-long
    ``list[float]`` indexed by the number of rises ``r = 0..K - 1``,
    unrounded. The result sums to 1.
    """
    impacts = _validate_matrix("H", H, non_negative=False)
    uncertainties = _validate_matrix("W", W)
    ...
    for k in range(n_scenarios):
        ...
        for i in range(n_receptors):
            ...
            d = [
                math.fsum((d[r] * (1.0 - p), d[r - 1] * p if r > 0 else 0.0))
                for r in range(n_receptors + 1)
            ]
        count_pmfs.append(d)
    ...
    for k in range(1, n_scenarios):
        ...
        for (c, r) in sorted(state):
            ...
            for c_next in range(n_receptors + 1):
                ...
                updated.setdefault(target, []).append(mass * pmf[c_next])
        state = {
            target: math.fsum(terms) for target, terms in updated.items()
        }
