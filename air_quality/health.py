...
def risk_excess_cvar(
    ...
    """Upper-tail conditional value at risk of aggregate expected excess.
    ...
    weights: ``None`` (the default) or a K-long list or tuple of finite,
        ...
        every scenario has weight ``1 / K``; otherwise the weights are
        normalized by their ``fsum``.
    Returns ``C`` where, with ``mu[k] = fsum(H[k])`` and
    ``sigma[k] = hypot(*W[k])``:
    ...
    * when ``sigma[k] > 0``, with ``a = (thresholds[j] - mu[k]) / sigma[k]``:
      ...
      ``E[k][j] = sigma[k] * exp(-a * a / 2) / sqrt(2 * pi)
      + (mu[k] - thresholds[j]) * p``;
    * when ``sigma[k] == 0``: ``E[k][j] = max(mu[k] - thresholds[j], 0.0)``;
    * for each threshold ``j`` and quantile ``q`` the scenarios are sorted
      by ``(E[k][j], -k)`` descending; with ``remaining = 1 - q`` the
      scenarios are visited in that order, contributing
      ``take = min(w[k], remaining)`` each until ``remaining`` reaches 0,
      and ``C[m][j] = fsum(take * E[k][j]) / (1 - q)``.
    ...
    C: list[list[float]] = [[0.0] * 3 for _ in qs]
    for j in range(3):
        values = [E[k][j] for k in range(n_scenarios)]
        order = sorted(
            range(n_scenarios), key=lambda k: (values[k], -k), reverse=True
        )
        for m, q in enumerate(qs):
