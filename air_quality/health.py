...
def receptor_count_probability(
    ...
    """Probability distribution of the number of receptors exceeding thresholds.
    ...
    * for each ``(k, j)`` the Poisson-binomial distribution over the
      receptor count is built by dynamic programming: starting with
      ``d = [1.0] + [0.0] * N``, each receptor updates
      ``n[r] = fsum((d[r] * (1 - q), d[r - 1] * q))`` with out-of-range
      terms taken as ``0.0``, giving ``d[k][j][r]``, the probability that
      exactly ``r`` of the ``N`` receptors exceed ``thresholds[j]``;
    * ``p[j][r] = fsum(w[k] * d[k][j][r] for k in range(K))``.

    ``p`` is a 3 x (N + 1) list of lists of floats, in threshold then
    count (``r = 0 ... N``) order, unrounded.
    """
    impacts = _validate_matrix("H", H, non_negative=False)
    uncertainties = _validate_matrix("W", W)
        ...
        for level in levels:
            ...
            for i in range(n_receptors):
                ...
                for r in range(n_receptors + 1):
                    rise = d[r - 1] * q if r > 0 else 0.0
                    n[r] = math.fsum((d[r] * (1.0 - q), rise))
                d = n
            per_level.append(d)
        distributions.append(per_level)
