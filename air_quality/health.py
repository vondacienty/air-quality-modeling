...
def receptor_level_count_quantile(
    ...
    * ``P[l][r] = fsum(w[k] * d[k][l][r] for k in range(K))``;
    * ``Q[m][l]`` is the smallest count ``r`` whose cumulative
      probability ``fsum(P[l][:r + 1])`` is ``>= quantiles[m]``
      (``0`` when ``quantiles[m] == 0``).

    ``Q`` is an M x 4 list of lists of ints, in ``quantiles`` input then
    level (``l = 0..3``) order.
    """
    impacts = _validate_matrix("H", H, non_negative=False)
    uncertainties = _validate_matrix("W", W)
    ...
    for q in qs:
        ...
        for level_index in range(4):
            if q == 0.0:
                ...
                chosen = n_receptors
                for r in range(n_receptors + 1):
                    if math.fsum(P[level_index][s] for s in range(r + 1)) >= q:
                        chosen = r
                        break
                row.append(chosen)
        Q.append(row)
