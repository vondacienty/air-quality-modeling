...
def receptor_level_run_cvar(
    ...
    """Upper-tail conditional value at risk of the longest run of each level.
    ...
    * ``p[k][i] = [1 - q0, q0 - q1, q1 - q2, q2]`` holds the probabilities
    ...
    other event with probability ``1 - p[k][i][l]`` moves to ``(0, m)``;
    probabilities landing on the same target state are combined with
    ``math.fsum``. The terminal states give
    ``G[m] = fsum(D[(r, m)] for r = 0..K)``, the probability mass on
    longest run ``m``. For each quantile ``q`` the lengths are visited
    from ``m = K`` down to ``0``, each contributing ``m * take`` with
    ``take = min(G[m], 1 - q)`` accumulated via ``math.fsum``, and the
    total is divided by ``1 - q``.

    Returns an M x N x 4 ``list[list[list[float]]]`` indexed by quantile,
    then receptor, then level, unrounded.
    """
    impacts = _validate_matrix("H", H, non_negative=False)
    uncertainties = _validate_matrix("W", W)
    ...
    for i in range(n_receptors):
        for l in range(4):
            ...
            for (r, m), probability in state.items():
                longest_terms[m].append(probability)
            longest = [math.fsum(terms) for terms in longest_terms]
            for mi, q in enumerate(qs):
                num = 0.0
                for m in range(n_scenarios, -1, -1):
                    take = min(longest[m], 1.0 - q)
                    num = math.fsum((num, m * take))
                result[mi][i][l] = num / (1.0 - q)
