...
def receptor_level_run_entropy(
    ...
    """Shannon entropy (natural log) of the longest level-run law per receptor.
    ...
    * ``p[k][i] = [1 - q0, q0 - q1, q1 - q2, q2]`` holds the probabilities
    ...
    ``E[i][l] = -fsum(G[m] * log(G[m]) for m = 0..K if G[m] > 0)``.
    ...
    for i in range(n_receptors):
        ...
        for l in range(4):
            ...
            G = [math.fsum(terms) for terms in longest_terms]
            receptor.append(
                -math.fsum(g * math.log(g) for g in G if g > 0)
            )
        result.append(receptor)
