...
def receptor_level_quantile(
    ...
    """Weighted per-receptor quantiles of the four health levels.
    ...
    * ``q[j] = 0.5 * erfc((t - mu) / (sigma * sqrt(2)))`` when
      ``sigma > 0``, otherwise ``1.0`` if ``t <= mu`` else ``0.0``;
    * ``p[k][i] = [1 - q0, q0 - q1, q1 - q2, q2]`` holds the probabilities
      of the four health levels for scenario ``k`` at receptor ``i``;
    * ``P[i][r] = fsum(w[k] * p[k][i][r] for k in range(K))`` is the
      weighted probability of level ``r`` at receptor ``i``;
    * ``Q[m][i]`` is ``0`` when ``quantiles[m] == 0``, otherwise the
      smallest level ``r`` whose cumulative probability
      ``fsum(P[i][s] for s in range(r + 1))`` is
      ``>= quantiles[m]``.
    ...
    P = [
        [
            math.fsum(w[k] * per_scenario[k][i][r] for k in range(n_scenarios))
            for r in range(4)
        ]
        for i in range(n_receptors)
    ]
    ...
    for q in qs:
        ...
        for i in range(n_receptors):
            if q == 0.0:
                ...
                chosen = 3
                for r in range(4):
                    cumulative = math.fsum([cumulative, P[i][r]])
                    if cumulative >= q:
                        chosen = r
                        break
                row.append(chosen)
