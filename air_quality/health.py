...
def receptor_risk_interval(
    ...
    """Per-receptor aggregate impacts with intervals and exceedance risks.
    ...
    * ``R[i] = z * sqrt(V[i])``,
    * ``lower[i] = M[i] - R[i]``, ``upper[i] = M[i] + R[i]``,
    * ``q[k][j] = 0.5 * erfc((thresholds[j] - H[k][i])
      / (W[k][i] * sqrt(2)))`` when ``W[k][i] > 0``, otherwise ``1.0`` if
      ``thresholds[j] <= H[k][i]`` else ``0.0``;
    * ``probabilities[i][j] = fsum(w[k] * q[k][j] for k in range(K))``.
