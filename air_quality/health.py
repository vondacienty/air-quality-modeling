...
def sensitivity(
    ...
    """Decompose aggregated scenario impacts into per-scenario contributions.
    ...
    z: number of standard deviations for the interval half-width; finite
        and >= 0 (default 1.96).
    Returns ``(M, R, C, S)`` where, per receptor ``i`` and scenario ``k``:
    ...
    * ``S[k][i] = 0.0`` when ``V[i] == 0``, otherwise
      ``S[k][i] = w[k] * W[k][i] ** 2 / V[i]``.

    ``M`` and ``R`` are N-long lists of floats and ``C`` and ``S`` are
    K x N lists of lists of floats, all in input order and unrounded.
    """
    impacts = _validate_matrix("H", H, non_negative=False)
    uncertainties = _validate_matrix("W", W)
    ...
    R = [z * math.sqrt(variance) for variance in V]
    C = [
        [w[k] * (impacts[k][i] - M[i]) for i in range(n_receptors)]
        for k in range(n_scenarios)
    ]
    S = [
        [
            0.0 if V[i] == 0 else w[k] * uncertainties[k][i] ** 2 / V[i]
            for i in range(n_receptors)
        ]
        for k in range(n_scenarios)
    ]
