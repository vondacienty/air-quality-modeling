...
def receptor_excess_cvar(
    ...
    """Weighted conditional value at risk of per-receptor expected excess.
    ...
    * for each receptor ``i``, threshold ``j`` and quantile ``q`` the
      scenarios are sorted by ``(e[k][i][j], -k)`` descending; starting
      with ``remaining = 1 - q``, each scenario in that order contributes
      ``take = min(w[k], remaining)``, accumulating
      ``fsum(take * e[k][i][j])`` while decrementing ``remaining`` by
      ``take`` and stopping once it reaches ``0``;
    * ``Q[m][i][j]`` is that accumulation divided by ``1 - q``.
        ...
        for j in range(3):
            ...
            for m, q in enumerate(qs):
                ...
                for k in order:
                    ...
                    terms.append(take * excesses[k][i][j])
                    remaining -= take
                    if remaining == 0.0:
                        break
                Q[m][i][j] = math.fsum(terms) / (1.0 - q)
