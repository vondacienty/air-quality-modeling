...
def expected_excess(
    ...
    """Aggregate exceedance probabilities and expected excesses over thresholds.
    ...
    ``mu[k] = fsum(H[k])`` and ``sigma[k] = hypot(*W[k])``, per scenario
    ``k`` and threshold index ``j``:

    * when ``sigma[k] > 0``, with ``a = (thresholds[j] - mu[k]) / sigma[k]``:
      ``p[k][j] = erfc(a / sqrt(2)) / 2`` and
    ...
    * ``excess[j] = fsum(w[k] * e[k][j] for k in range(K))``.
    ...
    Both results are 3-long lists of floats in ``thresholds`` order,
    unrounded.
    """
    impacts = _validate_matrix("H", H, non_negative=False)
    uncertainties = _validate_matrix("W", W)
    ...
    per_scenario_p: list[list[float]] = []
    per_scenario_e: list[list[float]] = []
    for k in range(n_scenarios):
        mu = math.fsum(impacts[k])
        sigma = math.hypot(*uncertainties[k])
        p_row: list[float] = []
        ...
        for level in levels:
            if sigma > 0:
                a = (level - mu) / sigma
                p = math.erfc(a / math.sqrt(2)) / 2
                e = (
                    sigma * math.exp(-a * a / 2) / math.sqrt(2 * math.pi)
                    + (mu - level) * p
                )
            else:
            ...
            p_row.append(p)
            e_row.append(e)
        per_scenario_p.append(p_row)
        per_scenario_e.append(e_row)

    probabilities = [
        math.fsum(w[k] * per_scenario_p[k][j] for k in range(n_scenarios))
        for j in range(3)
    ]
    excess = [
        math.fsum(w[k] * per_scenario_e[k][j] for k in range(n_scenarios))
        for j in range(3)
    ]
