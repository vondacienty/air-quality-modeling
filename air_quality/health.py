...
def receptor_level_path_probability(
    ...
    """Per-receptor probabilities of prescribed health-level paths.

    Scenario errors are treated as independent, so the probability of a
    path is the product of the per-scenario level probabilities along it.
    ...
    thresholds: list or tuple of exactly 3 finite, non-negative, strictly
        increasing numbers.
    paths: non-empty list or tuple of M paths; each ``paths[m]`` is a list
        or tuple of exactly K non-bool ints with ``0 <= level <= 3``.
    With ``mu = H[k][i]`` and ``sigma = W[k][i]``:
    ...
    * ``p[k][i] = [1 - q0, q0 - q1, q1 - q2, q2]`` holds the probabilities
      of the four health levels for scenario ``k`` at receptor ``i``;
    * ``result[m][i] = math.prod(p[k][i][paths[m][k]] for k in range(K))``.

    Returns an M x N list of lists of floats in path/receptor order,
    unrounded.
    """
    impacts = _validate_matrix("H", H, non_negative=False)
    uncertainties = _validate_matrix("W", W)
    ...
    for m, path in enumerate(paths):
        ...
        for k, item in enumerate(path):
            if not isinstance(item, int) or isinstance(item, bool):
                raise TypeError(
                    f"paths[{m}][{k}] must be an int, got {type(item).__name__}"
                )
            if not 0 <= item <= 3:
                raise ValueError(
