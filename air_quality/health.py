...
def risk_interval(
    ...
    """Aggregate scenario impacts into a risk interval and exceedance risks.
    ...
    * ``M = fsum(w[k] * mu[k] for k in range(K))``;
    * ``V = fsum(w[k] * ((mu[k] - M) ** 2 + sigma[k] ** 2))``;
    * ``R = z * sqrt(V)``, ``lower = M - R``, ``upper = M + R``;
    * ``q[k][j] = 0.5 * erfc((thresholds[j] - mu[k]) / (sigma[k] * sqrt(2)))``
      when ``sigma[k] > 0``, otherwise ``1.0`` if
      ``thresholds[j] <= mu[k]`` else ``0.0``;
    * ``p[j] = fsum(w[k] * q[k][j] for k in range(K))``.

    ``M``, ``R``, ``lower`` and ``upper`` are floats and ``p`` is a 3-long
    list of floats, all unrounded.
    """
    impacts = _validate_matrix("H", H, non_negative=False)
    uncertainties = _validate_matrix("W", W)
    ...
    sigma: list[float] = []
    for k in range(n_scenarios):
        mu.append(math.fsum(impacts[k]))
        sigma.append(math.hypot(*uncertainties[k]))

    mean = math.fsum(w[k] * mu[k] for k in range(n_scenarios))
    variance = math.fsum(
        w[k] * ((mu[k] - mean) ** 2 + sigma[k] ** 2) for k in range(n_scenarios)
    )
    ...
    for k in range(n_scenarios):
        ...
        for level in levels:
            if sigma[k] > 0:
                row.append(0.5 * math.erfc((level - mu[k]) / (sigma[k] * math.sqrt(2))))
            else:
                row.append(1.0 if level <= mu[k] else 0.0)
        per_scenario.append(row)
    ...
    return mean, spread, mean - spread, mean + spread, probabilities
