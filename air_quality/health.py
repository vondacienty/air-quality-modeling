...
def risk_excess_quantile(
    ...
    """Weighted quantiles of aggregate threshold excess.
    ...
    * for each threshold ``j`` the scenarios are sorted by
      ``(E[k][j], k)`` ascending and, for each requested quantile ``q``,
      the quantile scenario is the first one whose cumulative normalized
      weight is ``>= q`` (the first scenario in that order when
      ``q == 0``);
    * ``Q[m][j]`` holds that scenario's ``E`` value.
    ...
    for k in range(n_scenarios):
        ...
        for level in levels:
            if sigma > 0:
                a = (level - mu) / sigma
                phi = math.exp(-a * a / 2.0) / sqrt_2pi
                e = sigma * phi + (mu - level) * math.erfc(a / math.sqrt(2)) / 2.0
            else:
                e = max(mu - level, 0.0)
            row.append(e)
        excesses.append(row)
