...
def receptor_level_transition(
    ...
    """Adjacent-scenario health-level transition matrices, receptors equal.

    Scenario errors are independent and every receptor carries equal
    weight ``1 / N``.

    H: scenario x receptor matrix of health impacts with at least 2
        scenarios; both the outer container and each row must be a list
        or tuple, rows must be non-empty and share one receptor count;
        each value finite (negative values allowed).
    W: matrix of uncertainties with the same shape as ``H``; each value
        finite and >= 0.
    thresholds: list or tuple of exactly 3 finite, non-negative, strictly
        increasing numbers.
    ...
    * ``q[j] = 0.5 * erfc((thresholds[j] - mu) / (sigma * sqrt(2)))``
      when ``sigma > 0``, otherwise ``1.0`` if ``thresholds[j] <= mu``
      else ``0.0``;
    * ``p[k][i] = [1 - q0, q0 - q1, q1 - q2, q2]`` holds the
      probabilities of the four health levels for receptor ``i`` under
      scenario ``k``;
    * for ``t = 0..K-2`` and ``a, b = 0..3``,
      ``T[t][a][b] = fsum(p[t][i][a] * p[t+1][i][b] for i in range(N)) / N``.

    Returns a ``(K-1) x 4 x 4`` ``list[list[list[float]]]`` ordered by
    ``t``, ``a``, ``b``, all values unrounded.
    """
    impacts = _validate_matrix("H", H, non_negative=False)
    uncertainties = _validate_matrix("W", W)
    ...
    if n_scenarios < 2:
        raise ValueError(
            f"H and W must have at least 2 scenarios, got {n_scenarios}"
        )
    n_receptors = len(impacts[0])
    for k, row in enumerate(uncertainties):
        if len(row) != n_receptors:
