...
def receptor_count_cvar(
    ...
    """Upper-tail conditional value at risk of the exceedance-receptor count.
    ...
    * ``d[k][j]`` is the (N + 1)-long probability mass function of the
      ...
      ``j``: it starts as ``[1.0, 0.0, ...]`` and folds the receptors in
      one at a time with
      ``n[r] = fsum((d[r] * (1 - q), d[r - 1] * q))`` (the second term
      taken as 0 when ``r == 0``);
    * with the weights normalized via ``fsum``, each atom
      ``(w[k] * d[k][j][r], r, k)`` is visited in ascending ``(-r, k)``
      order; for each quantile ``q``, with ``remaining = 1 - q``, the
      atoms contribute ``take = min(atom probability, remaining)`` in that
      order, accumulating
    ...
    atoms: list[list[tuple[float, int, int]]] = []
    for j in range(3):
        entries = [
        ...
        entries.sort(key=lambda atom: (-atom[1], atom[2]))
        atoms.append(entries)
    ...
    for q in qs:
        ...
        for j in range(3):
            remaining = tail
            num = 0.0
            for probability, r, _ in atoms[j]:
                if remaining <= 0.0:
                    break
