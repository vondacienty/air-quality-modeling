...
def receptor_level_run_stats(
    ...
    Scenario errors are treated as independent and scenarios are folded in
    their given order, independently for every receptor and level; a "run"
    is a maximal stretch of consecutive scenarios at the same level, and a
    run "qualifies" once its length reaches ``minimum``.
    ...
    thresholds: list or tuple of exactly 3 finite, non-negative, strictly
        increasing numbers.
    minimum: qualifying run length; a non-bool int >= 1.
    With ``mu = H[k][i]`` and ``sigma = W[k][i]``:
    ...
    * ``p[k][i] = [1 - q0, q0 - q1, q1 - q2, q2]`` holds the probabilities
    ...
    For each receptor ``i`` and level ``l`` a recursion over states
    ``(r, c)`` (current run length ``r`` truncated at ``minimum``, and the
    number ``c`` of completed qualifying runs) starts from
    ``D[(0, 0)] = 1.0``. Scenarios are processed in order; with state keys
    in lexicographic order, an ``l`` event with probability
    ``p[k][i][l]`` moves to ``(min(r + 1, minimum), c + 1)`` when
    ``r < minimum`` and ``r + 1 == minimum`` and to
    ``(min(r + 1, minimum), c)`` otherwise, while any other event with
    probability ``1 - p[k][i][l]`` moves to ``(0, c)``; probabilities
    landing on the same target state are combined with ``math.fsum``.
    The results are ``probability[i][l] = fsum(D[(r, c)] for c >= 1)`` and
    ``expected_runs[i][l] = fsum(c * D[(r, c)])``.
