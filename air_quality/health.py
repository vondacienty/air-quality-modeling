...
def receptor_expected_excess(
    ...
    """Per-receptor weighted exceedance probability and expected excess.
    ...
    * when ``sigma > 0``, with ``a = (thresholds[j] - mu) / sigma``:
      ``p = 0.5 * erfc(a / sqrt(2))`` and
      ``e = sigma * exp(-a * a / 2) / sqrt(2 * pi) + (mu - thresholds[j]) * p``;
    * when ``sigma == 0``: ``p = 1.0`` if ``thresholds[j] <= mu`` else
      ``0.0``, and ``e = max(mu - thresholds[j], 0.0)``;
    ...
    * ``excess[i][j] = fsum(w[k] * e for k in range(K))``.
    ...
    for i in range(n_receptors):
        probabilities.append(
            [
                math.fsum(w[k] * per_p[k][i][j] for k in range(n_scenarios))
                for j in range(3)
            ]
        )
        excess.append(
            [
                math.fsum(w[k] * per_e[k][i][j] for k in range(n_scenarios))
                for j in range(3)
            ]
        )
