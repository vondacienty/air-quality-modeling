...
def receptor_level_probability(
    ...
    """Aggregate health-level probabilities per receptor across scenarios.
    ...
    thresholds: list or tuple of exactly 3 finite, non-negative, strictly
        increasing numbers.
    Scenarios are weighted equally (``1 / K`` each). Returns
    ``(probabilities, expected_level, alert_probability)`` where, per
    scenario ``k`` and receptor ``i`` with ``mu = H[k][i]`` and
    ``sigma = W[k][i]``:

    * ``q[j] = 0.5 * erfc((thresholds[j] - mu) / (sigma * sqrt(2)))``
      when ``sigma > 0``, otherwise ``1.0`` if
      ``thresholds[j] <= mu`` else ``0.0``;
        ...
        for i in range(n_receptors):
            ...
            for level in levels:
                if sigma > 0:
                    ...
                    q.append(1.0 if level <= mu else 0.0)
            scenario_rows.append([1.0 - q[0], q[0] - q[1], q[1] - q[2], q[2]])
        per_scenario.append(scenario_rows)
