...
def receptor_level_event_probability(
    ...
    """Probability that per-level receptor counts all reach given minimums.
    ...
    minimums: list or tuple of exactly 4 non-bool ints, each >= 0; entry
        ``l`` is the minimum required number of receptors at health level
        ``l``.
    weights: ``None`` (the default) or a K-long list or tuple of finite,
        non-negative entries whose ``fsum`` is positive. With ``None``
        every scenario has weight ``1 / K``; otherwise the weights are
    ...
    * ``p[k][i] = [1 - q0, q0 - q1, q1 - q2, q2]`` holds the probabilities
      of the four health levels for scenario ``k`` at receptor ``i``;
    * the joint distribution ``D`` of the per-level receptor counts
      ``(c0, c1, c2, c3)`` starts as ``{(0, 0, 0, 0): 1.0}`` and folds the
      receptors in one at a time: for each state ``c`` (visited in
      lexicographic order) and each level ``l`` in ``0..3``,
      ``D'[c + e_l] += D[c] * p[k][i][l]``, with the contributions to each
      new state combined by ``math.fsum``;
    * ``A[k] = fsum(D[c] for c with c[l] >= minimums[l] for all l)``;
    * ``weighted = fsum(w[k] * A[k] for k in range(K))``.

    ``A`` is a K-long list of floats and ``weighted`` is a float, both in
    scenario order and unrounded.
    """
    impacts = _validate_matrix("H", H, non_negative=False)
    uncertainties = _validate_matrix("W", W)
    ...
    A: list[float] = []
    for k in range(n_scenarios):
        counts: dict[tuple[int, int, int, int], float] = {(0, 0, 0, 0): 1.0}
        for i in range(n_receptors):
            mu = impacts[k][i]
            sigma = uncertainties[k][i]
            q: list[float] = []
            ...
            contributions: dict[tuple[int, int, int, int], list[float]] = {}
            for state in sorted(counts):
                base = counts[state]
                for level_index in range(4):
                    nxt = list(state)
                    nxt[level_index] += 1
                    contributions.setdefault(tuple(nxt), []).append(
                        base * p[level_index]
                    )
            counts = {
                state: math.fsum(parts) for state, parts in contributions.items()
            }
        A.append(
            math.fsum(
                value
                for state, value in counts.items()
                if all(state[l] >= required[l] for l in range(4))
            )
        )
