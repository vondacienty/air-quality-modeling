...
def excess_interval(
    ...
    z: number of standard deviations for the interval half-width; finite
        and >= 0 (default 1.96).
    Returns ``(M, R, lower, upper)`` where, with ``mu[k] = fsum(H[k])``,
    ``sigma[k] = hypot(*W[k])``, ``t = thresholds[j]`` and
    ``d = mu[k] - t``:
    ...
    * when ``sigma[k] == 0``: ``e[k][j] = max(d, 0.0)`` and
      ``s[k][j] = 0.0``;
    * ``M[j] = fsum(w[k] * e[k][j] for k in range(K))``;
    * ``V[j] = fsum(w[k] * ((e[k][j] - M[j]) ** 2 + s[k][j])
      for k in range(K))``;
    * ``R[j] = z * sqrt(V[j])``;
    * ``lower[j] = max(M[j] - R[j], 0.0)``, ``upper[j] = M[j] + R[j]``.
    ...
    upper: list[float] = []
    for j in range(3):
        mean = math.fsum(w[k] * per_e[k][j] for k in range(n_scenarios))
        variance = math.fsum(
            w[k] * ((per_e[k][j] - mean) ** 2 + per_s[k][j])
            for k in range(n_scenarios)
        )
