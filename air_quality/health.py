...
def risk_share(
    ...
    """Each scenario's share of aggregate exceedance risk and expected excess.
    ...
    * when ``sigma[k] > 0``, with ``t = thresholds[j]`` and
      ``a = (t - mu[k]) / sigma[k]``:
      ``q[k][j] = 0.5 * erfc(a / sqrt(2))`` and
      ``e[k][j] = sigma[k] * exp(-a * a / 2) / sqrt(2 * pi)
      + (mu[k] - t) * q[k][j]``;
    * when ``sigma[k] == 0``: ``q[k][j] = 1.0`` if ``t <= mu[k]`` else
      ``0.0``, and ``e[k][j] = max(mu[k] - t, 0.0)``;
    ...
    * ``PS[k][j] = p[k][j] / P[j]`` and ``ES[k][j] = r[k][j] / E[j]``,
      each taken as ``0.0`` when its denominator is zero.
    ...
    P = [math.fsum(p[k][j] for k in range(n_scenarios)) for j in range(3)]
    E = [math.fsum(r[k][j] for k in range(n_scenarios)) for j in range(3)]
    ...
    for k in range(n_scenarios):
        ...
        es_row: list[float] = []
        for j in range(3):
            ps_row.append(p[k][j] / P[j] if P[j] != 0.0 else 0.0)
            es_row.append(r[k][j] / E[j] if E[j] != 0.0 else 0.0)
        PS.append(ps_row)
        ES.append(es_row)
