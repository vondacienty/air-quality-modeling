...
def receptor_level_count_share(
    ...
    """Per-scenario share of the probability and expected count at each level.
    ...
    * ``A[k][l][r] = w[k] * d[k][l][r]`` and
      ``P[l][r] = fsum(A[k][l][r] for k in range(K))``;
    ...
    * ``ES[k][l][r] = A[k][l][r] * r / B[l]``, taken as ``0.0`` when
    ...
    unrounded. Summing ``PS`` over scenarios yields 1 for every
    ``(l, r)`` whose ``P[l][r]`` is positive, and summing ``ES`` over
    scenarios and counts yields 1 for every level whose ``B[l]`` is
    positive.
    """
    impacts = _validate_matrix("H", H, non_negative=False)
    uncertainties = _validate_matrix("W", W)
    ...
    A = [
        [
            [w[k] * distributions[k][l][r] for r in range(n_receptors + 1)]
            for l in range(4)
        ]
        for k in range(n_scenarios)
    ]
    P = [
        [
            math.fsum(A[k][l][r] for k in range(n_scenarios))
            for r in range(n_receptors + 1)
        ]
        for l in range(4)
    ]
    B = [
        math.fsum(r * P[l][r] for r in range(n_receptors + 1)) for l in range(4)
    ]
        ...
        for l in range(4):
            ...
            for r in range(n_receptors + 1):
                if P[l][r] == 0.0:
                    ...
                    ps_row.append(A[k][l][r] / P[l][r])
                if B[l] == 0.0:
                    es_row.append(0.0)
                else:
                    es_row.append(A[k][l][r] * r / B[l])
