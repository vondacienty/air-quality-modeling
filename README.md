...
- 健康影响评估模块 `air_quality.health`（通过 `air_quality.health.xxx` 调用，不在包根导出），提供以下函数：
  ...
  - `receptor_level_rise_distribution(H, W, thresholds)`：分受体健康等级在相邻情景间上升次数的分布（情景误差独立）。`H`、`W` 为同形 K×N（K≥2,N>0）list/tuple 矩阵，外层/行限 list/tuple 且等宽；`H` 元素为非 bool 有限 int/float（可负），`W` 元素为非 bool 有限 int/float 且 ≥0；`thresholds` 为 list/tuple，恰含 3 个严格递增非 bool 有限非负数。类型错抛 `TypeError`；空、空行、锯齿、K<2、形状不符、非有限、W 负、thresholds 长度/顺序/负值非法抛 `ValueError`。令 `μ=H[k][i]`、`σ=W[k][i]`、`Tj=thresholds[j]`：`σ>0` 时 `qj=erfc((Tj−μ)/(σ*sqrt(2)))/2`，否则 `Tj≤μ` 取 `1.0`、否则 `0.0`；`p=[1−q0, q0−q1, q1−q2, q2]`。每受体 `i` 令 `D[r][0]=p[0][i][r]`（其余为 0），对 `k=1..K−1`、`c=0..k` 按 `D′[b][c]=fsum(D[a][c−(a<b)]*p[k][i][b] for a=0..3)`（越界项略）更新 `D=D′`。返回 `R[i][c]=fsum(D[r][c] for r=0..3)` 的 N×K `list[list[float]]`，不舍入
