...
- 健康影响评估模块 `air_quality.health`（通过 `air_quality.health.xxx` 调用，不在包根导出），提供以下函数：
  ...
  - `risk_excess_quantile(H, W, thresholds, quantiles, weights=None)`：情景总期望超额在跨情景分布上的加权分位数。`H`、`W` 为同形 K×N（K,N>0）list/tuple 矩阵，元素为非 bool 有限 int/float（`H` 可负，`W` 元素 ≥0）；`thresholds` 恰为 3 个严格递增的非 bool 有限非负数；`quantiles` 为非空 list/tuple，元素为非 bool 有限数且 `0≤q<1`、非递减；`weights` 为 `None`（取等权 `1/K`）或 K 长 list/tuple，元素为非 bool 有限非负数且 `fsum>0`（按 `fsum` 归一）。容器/元素类型错抛 `TypeError`；空、锯齿、形状/长度不符、非有限、负值、阈值/分位越界或递减、权和非法抛 `ValueError`。令 `μk=fsum(H[k])`、`σk=hypot(*W[k])`、`t=thresholds[j]`：`σk>0` 时 `a=(t−μk)/σk`、`e=σk*exp(−a²/2)/sqrt(2π)+(μk−t)*erfc(a/sqrt(2))/2`；`σk=0` 时 `e=max(μk−t,0)`。对每 `(j,q)` 将各情景按 `(e[k][j],k)` 升序排列，`q=0` 取首项，否则取累计归一权首次 `>=q` 的情景。返回分位×阈值序 M×3 `list[list[float]]`，不舍入
