...
- 健康影响评估模块 `air_quality.health`（通过 `air_quality.health.xxx` 调用，不在包根导出），提供以下函数：
  ...
  - `receptor_level_count_quantile(H, W, thresholds, quantiles, weights=None)`：假定同一情景内各受体误差独立时，处于各健康等级的受体数的跨情景加权分位数（一热向量形式）。`H`、`W` 为同形 K×N（K,N>0）list/tuple 矩阵，`H` 元素为非 bool 有限数（可负），`W` 元素为非 bool 有限非负数；`thresholds` 为恰 3 个严格递增的非 bool 有限非负数；`quantiles` 为非空、`[0,1]` 内非递减的非 bool 有限数序列；`weights` 为 `None`（取等权）或 K 长非负有限序列且 `fsum>0`（按 `fsum` 归一）。容器/元素类型错抛 `TypeError`；空、锯齿、形状长度、非有限、负值、阈值/分位顺序或权和非法抛 `ValueError`。对 `μ=H[k][i]`、`σ=W[k][i]`：`σ>0` 时 `qj=erfc((thresholds[j]−μ)/(σ*sqrt(2)))/2`，否则 `qj=1.0`（`thresholds[j]≤μ`）或 `0.0`；`p[k][i]=[1−q0, q0−q1, q1−q2, q2]`。对每个 `(k,l)`（`l=0..3`），`d` 自 `[1]+[0]*N` 起逐受体按 `d′[r]=d[r]*(1−p[k][i][l])+(r>0 ? d[r−1]*p[k][i][l] : 0)` 递推，`P[l][r]=fsum(w[k]*d[k][l][r])`。返回 `R[m][l]` 为 N+1 长的 int 一热向量：`q=0` 时 `r=0`，否则 `r` 取最小使 `fsum(P[l][0..r])>=quantiles[m]` 的计数，该位为 `1`、其余为 `0`；整体形状 M×4×(N+1)，按分位、等级、计数序，不舍入
