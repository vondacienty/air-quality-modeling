...
- 健康影响评估模块 `air_quality.health`（通过 `air_quality.health.xxx` 调用，不在包根导出），提供以下函数：
  ...
  - `receptor_level_transition(H, W, thresholds)`：情景误差独立、受体等权时的相邻情景健康等级转移矩阵。`H`、`W` 为同形 K×N（K≥2，N>0）list/tuple 矩阵，外层与各行均限 list/tuple 且等宽，元素为非 bool 有限 int/float（`H` 可负，`W` 元素 ≥0）；`thresholds` 为恰含 3 个严格递增非 bool 有限非负数的 list/tuple。类型错抛 `TypeError`；空、空行、锯齿、K<2、形状不符、非有限、`W` 负值、阈值长度/顺序/负值非法抛 `ValueError`。对 `μ=H[k][i]`、`σ=W[k][i]`：`σ>0` 时 `qj=erfc((thresholds[j]−μ)/(σ*sqrt(2)))/2`，否则 `thresholds[j]≤μ` 取 `1.0`、否则 `0.0`；`p[k][i]=[1−q0, q0−q1, q1−q2, q2]`。对 `t=0..K−2`、`a,b=0..3`，`T[t][a][b]=fsum(p[t][i][a]*p[t+1][i][b] for i in 0..N−1)/N`。返回 (K−1)×4×4 的 `list[list[list[float]]]`，按 t、a、b 序，不舍入
