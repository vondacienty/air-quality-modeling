...
- 健康影响评估模块 `air_quality.health`（通过 `air_quality.health.xxx` 调用，不在包根导出），提供以下函数：
  ...
  - `receptor_excess_warning(H, W, thresholds, weights=None, z=1.96)`：由分受体期望超额区间派生的预警等级。五参与每种 `TypeError`/`ValueError` 逐项沿用 `receptor_excess_interval`（含容器、形状、有限性、非负性、阈值顺序、权和与 `z` 边界）。令 `(M,R,L,U)=receptor_excess_interval(H,W,thresholds,weights,z)`，则 `levels[i]=sum(L[i][j]>0 for j in 0..2)`、`triggers[i]=thresholds[levels[i]-1]`（`level=0` 时为 `None`）、`scores=M`、`interval=(L,U)`。返回 `(levels, triggers, scores, interval)`：`levels` 为 N 长 `list[int]`，`triggers` 为 N 长 `list[float|None]`，`scores` 为 N×3 浮点列表，`interval` 为二元 tuple 且两项均 N×3 浮点列表；按受体/阈值序，不舍入
