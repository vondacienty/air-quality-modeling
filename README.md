...
- 健康影响评估模块 `air_quality.health`（通过 `air_quality.health.xxx` 调用，不在包根导出），提供以下函数：
  ...
  - `receptor_level_event_probability(H, W, thresholds, minimums, weights=None)`：假定同一情景内各受体误差独立时，各健康等级受体数同时达到给定下限的事件概率。`H`、`W` 为同形 K×N（K,N>0）list/tuple 矩阵，元素为非 bool 有限数（`H` 可负，`W` 非负）；`thresholds` 恰为 3 个严格递增的非 bool 有限数；`minimums` 恰为 4 个非 bool 且 `>=0` 的 int；`weights` 为 `None`（取等权 `1/K`）或 K 个非负有限权且 `fsum>0`（按 `fsum` 归一）。容器/元素类型错抛 `TypeError`；空、锯齿、形状/长度不符、非有限、负值、阈值顺序、`minimums` 或权和非法抛 `ValueError`。对 `σ=W[k][i]`：`σ>0` 时 `qj=erfc((thresholds[j]−H[k][i])/(σ*sqrt(2)))/2`，否则 `thresholds[j]≤H[k][i]` 取 `1.0`、否则 `0.0`；`p=[1−q0, q0−q1, q1−q2, q2]`。联合计数分布 `D` 自 `{(0,0,0,0):1.0}` 起，逐受体对按字典序遍历的状态 `c` 与 `l=0..3` 作 `D′[c+e_l]+=D[c]*p_l`（各新状态的贡献以 `fsum` 合并）；`A[k]=fsum(D[c]，c_l≥minimums_l 对所有 l 成立)`，`weighted=fsum(w[k]*A[k])`。返回 `(A, weighted)`，`A` 为 K 长 `list[float]`，`weighted` 为 float，均不舍入
