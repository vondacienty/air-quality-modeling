...
- 健康影响评估模块 `air_quality.health`（通过 `air_quality.health.xxx` 调用，不在包根导出），提供以下函数：
  ...
  - `receptor_level_quantile(H, W, thresholds, quantiles, weights=None)`：分受体、跨情景加权的健康等级分位数。`H`、`W` 为同形 K×N 矩阵（`H` 元素可负，`W` 元素非负）；`thresholds` 为 3 个严格递增非负数；`quantiles` 为非空 list/tuple，元素在 `[0,1]` 且非递减；`weights` 为 `None`（取等权 `1/K`）或按 `fsum` 归一的 K 个非负权（`fsum` 须为正）。对 `μ=H[k][i]`、`σ=W[k][i]`、`t=thresholds[j]`：`σ>0` 时 `qj=0.5*erfc((t−μ)/(σ*sqrt(2)))`，`σ=0` 时 `t≤μ` 取 `1.0` 否则 `0.0`；`p[k][i]=[1−q0, q0−q1, q1−q2, q2]` 为情景 `k` 受体 `i` 四个健康等级的概率；`P[i][r]=fsum(w[k]*p[k][i][r] for k in range(K))`。分位 `q=0` 时取等级 `0`，否则取使 `fsum(P[i][s] for s in range(r+1))>=q` 的最小等级 `r`。返回分位×受体序的 M×N `list[list[int]]`
