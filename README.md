...
- 健康影响评估模块 `air_quality.health`（通过 `air_quality.health.xxx` 调用，不在包根导出），提供以下函数：
  ...
  - `receptor_excess_cvar(H, W, thresholds, quantiles, weights=None)`：分受体、分阈值的期望超额的跨情景加权条件风险价值（CVaR）。`H`、`W` 为同形 K×N（K,N>0）list/tuple 矩阵，元素为非 bool 有限 int/float（`H` 可负，`W` ≥0）；`thresholds` 恰为 3 个严格递增的非 bool 有限非负数；`quantiles` 为非空 list/tuple 数列，各项满足 `0≤q<1` 且非递减；`weights` 为 `None`（取等权 `1/K`）或 K 长 list/tuple，元素为非 bool 有限非负数且 `fsum>0`（按 `fsum` 归一）。容器/元素类型错抛 `TypeError`；空、锯齿、形状/长度不符、非有限、负值及阈值/分位/权和非法抛 `ValueError`。对 `μ=H[k][i]`、`σ=W[k][i]`、`t=thresholds[j]`：`σ>0` 时 `a=(t−μ)/σ`、`p=erfc(a/sqrt(2))/2`、`e=σ*exp(−a²/2)/sqrt(2π)+(μ−t)*p`；`σ=0` 时 `e=max(μ−t,0)`。对每 `(i,j,q)` 将各情景按 `(e[k][i][j],−k)` 降序排列；`remaining=1−q`，依序 `take=min(w[k],remaining)`，以 `math.fsum` 累计 `take*e[k][i][j]`，`remaining` 减去 `take` 至 `0` 停止，结果除以 `1−q`。返回分位/受体/阈值序的 M×N×3 `list[list[list[float]]]`，不舍入
