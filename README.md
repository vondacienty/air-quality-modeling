...
- 健康影响评估模块 `air_quality.health`（通过 `air_quality.health.xxx` 调用，不在包根导出），提供以下函数：
  ...
  - `receptor_mitigation_frontier(H, W, thresholds, options, budget, weights=None, z=1.96)`：预算内每受体选一缓解方案的非支配（Pareto 前沿）方案集。`options`/`budget` 契约及各类 `TypeError`/`ValueError` 与 `receptor_mitigation_plan` 相同，五参与 `receptor_excess_interval` 同契约。枚举每受体一个选项的组合，`C=fsum(c_i)≤budget` 方可行；令 `H′[k][i]=H[k][i]−r_i`，调用 `receptor_excess_interval(H′, W, thresholds, weights, z)` 得 `(M,R,L,U)`，并计算 `S=fsum(max(U[i][j],0))`、`D=fsum(R[i][j])`、`G=Σ[L[i][j]>0]`（`i<N, j<3`）。仅保留在 `(S,D,G,C)` 上逐项不劣且至少一项严格更优的方案；无可行组合抛 `ValueError`。返回按 `(S,D,G,C,choice)` 升序排列的 `list`，每项为 `(choice, C, S, D, G, (M,R,L,U))`，`choice` 为 N 长 0 基选项下标的 `list[int]`，均不舍入
