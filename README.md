...
- 健康影响评估模块 `air_quality.health`（通过 `air_quality.health.xxx` 调用，不在包根导出），提供以下函数：
  ...
  - `receptor_count_cvar(H, W, thresholds, quantiles, weights=None)`：假定同一情景内各受体误差独立时，超过各级阈值的受体数在跨情景混合分布上的上尾条件风险价值（CVaR）。`H`、`W` 为同形 K×N（K,N>0）list/tuple 矩阵，元素为非 bool 有限 int/float（`H` 可负，`W` 元素 ≥0）；`thresholds` 恰为 3 个严格递增的非 bool 有限非负数；`quantiles` 为非空 list/tuple，元素为非 bool 有限数且 `0≤q<1`、非递减，类型错抛 `TypeError`，越界或递减抛 `ValueError`；`weights` 为 `None`（取等权 `1/K`）或 K 长 list/tuple，元素为非 bool 有限非负数且 `fsum>0`（按 `fsum` 归一）；其余 `H`/`W`/`thresholds`/`weights` 及异常沿用 `receptor_count_interval`。对 `k,j,i`：`σ=W[k][i]>0` 时 `q_i=erfc((thresholds[j]−H[k][i])/(σ*sqrt(2)))/2`，否则 `thresholds[j]≤H[k][i]` 取 `1.0`、否则 `0.0`；`d=[1.0]+[0.0]*N` 逐受体递推 `n[r]=math.fsum((d[r]*(1−q_i),d[r−1]*q_i))`（`r=0` 第二项为 0）得 `d[k][j][r]`。原子 `(w[k]*d[k][j][r],r,k)` 按 `(-r,k)` 升序排列；每个 `q` 令 `rem=1−q`，依序 `take=min(原子概率,rem)`，`num=math.fsum((num,take*r))` 累加至 `rem=0`，结果为 `num/(1−q)`。返回分位×阈值序 M×3 `list[list[float]]`，不舍入
